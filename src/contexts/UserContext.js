...
export class UserProvider extends Component {
  ...
  getIndustries = () => {
    ApiService.getIndustries().then((res) => {
  ...
  getUnions = (page, search) => {
    ApiService.getUnions(page, search).then((res) => {
      const results = res;
  ...
  getUnionsByIndustry = (page, industry, search) => {
    ApiService.getUnionsByIndustry(page, industry, search).then((res) => {
      const results = res;
      this.setState({ results });
      this.setState({ currentPage: page });
    });
  ...
  goToPage = (page) => {
    const currentPage = Math.max(
      0,
      Math.min(page, this.state.results.pageCount)
    );
    this.setState({ currentPage });
    if (this.state.industry) {
      this.getUnionsByIndustry(
        currentPage,
        this.state.industry,
        this.state.search
      );
